...
// Keep production preparation limited to deterministic input normalization and
// security-related source fixes. Do NOT mutate the Push/VAPID implementation:
// pushNotificationService.ts owns its VAPID migration logic and must remain the
// exact source that was reviewed and committed. Mutating it during prebuild can
// re-introduce stale identifiers such as vapidVersion and make builds flaky.
...
patch('src/App.tsx', [
  ...
  // Never ship demo partner/order/coupon records. The database is the source of truth;
  // an empty local cache must remain empty until Supabase returns real data.
  ["      return saved ? JSON.parse(saved) : initialStores;", "      return saved ? JSON.parse(saved) : [];"],
  ["      return saved ? JSON.parse(saved) : [\n        {\n          id: 'merch-demo-1',\n          storeName: 'كافيه ومشويات السلطان',\n          businessType: 'مطعم',\n          ownerName: 'محمد أحمد',\n          phone: '01020304050',\n          city: 'القاهرة - الدقي',\n          notes: 'مطعم وجبات مشويات وطواجن شرقية',\n          status: 'pending',\n          createdAt: new Date().toISOString()\n        }\n      ];", "      return saved ? JSON.parse(saved) : [];"],
  ["      return saved ? JSON.parse(saved) : [\n        {\n          id: 'driver-demo-1',\n          fullName: 'كابتن ياسر محمود',\n          phone: '01122334455',\n          vehicleType: 'موتوسيكل',\n          vehicleModel: 'دايون 4 - 2023',\n          noLicense: false,\n          drivingLicenseNumber: 'EG-98214',\n          vehicleLicenseNumber: 'M-10293',\n          photoUrl: 'https://images.unsplash.com/photo-photo',\n          status: 'pending',\n          createdAt: new Date().toISOString()\n        }\n      ];", "      return saved ? JSON.parse(saved) : [];"],
  ...
  ["      return saved ? JSON.parse(saved) : [\n        {\n          id: '10928',", "      return saved ? JSON.parse(saved) : [];\n/* production: demo order removed */\n/*      return saved ? JSON.parse(saved) : [\n        {\n          id: '10928',"],
