...
export const CheckoutModal: React.FC<Props> = ({
  ...
  // Use the authenticated account phone as the default contact number.
  // The customer can override it for this order only.
  useEffect(() => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 animate-fade-in">
        <motion.div
          ...
          className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[94vh]"
        >
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-5 overflow-y-auto space-y-4 flex-1">
            ...
            <div className="space-y-3">
              <h4 className="font-extrabold text-xs text-slate-900 flex items-center gap-1.5">
                ...
                تفاصيل الوصول للعنوان:
              </h4>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[11px] font-bold text-slate-600 mb-1 block">العمارة / المنزل:</label>
                  <input type="text" required value={building} onChange={(e) => setBuilding(e.target.value)} placeholder="مثال: عمارة 12" className="w-full bg-slate-50 text-xs p-2.5 rounded-xl border border-slate-200 focus:border-orange-500 focus:outline-hidden" />
                </div>
                <div>
                  <label className="text-[11px] font-bold text-slate-600 mb-1 block">الدور والشقة:</label>
                  <input type="text" required value={floor} onChange={(e) => setFloor(e.target.value)} placeholder="مثال: الدور 3 شقة 6" className="w-full bg-slate-50 text-xs p-2.5 rounded-xl border border-slate-200 focus:border-orange-500 focus:outline-hidden" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="رقم التواصل من حسابك" inputMode="tel" autoComplete="tel" className="w-full bg-slate-50 text-xs p-2.5 pr-9 rounded-xl border border-slate-200 focus:border-orange-500 focus:outline-hidden" />
                </div>
                {currentUser?.phone ? (
              ...
              <div>
                ...
                <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="مثال: اتصل عند الوصول، الجرس معطل..." className="w-full bg-slate-50 text-xs p-2.5 rounded-xl border border-slate-200 focus:border-orange-500 focus:outline-hidden" />
              </div>
            </div>
