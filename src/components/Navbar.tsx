...
type Role = User['role'];
const VALID_ROLES: Role[] = ['customer', 'driver', 'merchant', 'admin'];
...
export const Navbar: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    async function loadRoles() {
      ...
      try {
        ...
        if (cancelled) return;
        const next = (data || [])
          .map((row: { role: string }) => row.role as Role)
        ...
        if (!cancelled) setRoles([currentUser.role]);
      }
    }
    void loadRoles();
  ...
  const switchRole = async (role: Role) => {
    ...
    if (!authData.user || authData.user.id !== currentUser.id) return;
    const { data: roleRow } = await supabase.from('user_roles').select('role').eq('user_id', currentUser.id).eq('role', role).maybeSingle();
    if (!roleRow) return;
  ...
  const isCustomerMode = !currentUser || currentUser.role === 'customer';
  const isDriverMode = currentUser?.role === 'driver';
  const isMerchantMode = currentUser?.role === 'merchant';
    ...
    <>
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs max-w-full overflow-visible">
        <div className="max-w-7xl mx-auto px-2.5 sm:px-4 py-2">
                      ...
                      {roleMenuOpen && (
                        <div className="absolute right-0 top-full mt-2 z-[100] w-[230px] max-w-[calc(100vw-16px)] bg-white border border-slate-200 rounded-2xl shadow-2xl p-2">
                          ...
                          {roles.map(role => {
                            ...
                            return <button key={role} type="button" disabled={active} onClick={() => void switchRole(role)} className={`w-full text-right px-2.5 py-2.5 rounded-xl flex items-center gap-2.5 transition-colors ${active ? 'bg-orange-50 text-orange-700 cursor-default' : 'text-slate-700 hover:bg-slate-50 hover:border-orange-200'}`}>
          ...
          {isCustomerMode && <div className="md:hidden mt-2 relative"><Search className="w-4 h-4 text-slate-400 absolute right-3.5 top-1/2 -translate-y-1/2 pointer-events-none" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="ابحث عن مطعم، سوبرماركت، وجبة أو منتج..." className="w-full bg-slate-100 focus:bg-white text-xs font-medium pr-10 pl-4 py-2 rounded-xl border border-slate-200/80 focus:border-orange-500 focus:outline-hidden transition-all shadow-inner text-slate-900" /></div>}
        </div>
      </header>

      {isDriverMode && currentUser && (
        <div className="fixed inset-x-0 top-[58px] sm:top-[68px] bottom-0 z-20 overflow-y-auto bg-slate-50">
          <DriverDashboard currentUser={currentUser} />
        </div>
      )}
      {isMerchantMode && currentUser && (
        <div className="fixed inset-x-0 top-[58px] sm:top-[68px] bottom-0 z-20 overflow-y-auto bg-slate-50">
          <MerchantDashboard
            currentUser={currentUser}
            onExit={() => {
              localStorage.setItem('talabak_active_role', 'customer');
              window.dispatchEvent(new CustomEvent('talabak-role-change', { detail: { role: 'customer' } }));
