...
import { Search, Download, ShoppingBag, User as UserIcon, ShieldCheck, LogOut, Sparkles, Bell, ChevronDown, ArrowLeft } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs max-w-full overflow-hidden">
      <div className="max-w-7xl mx-auto px-2.5 sm:px-4 py-2">
        ...
        {!currentUser && (
          <div className="mt-2.5 rounded-2xl border border-orange-200 bg-gradient-to-r from-orange-50 via-amber-50 to-orange-50 px-3 py-2.5 sm:px-4 sm:py-3 shadow-sm">
            <button onClick={onOpenAuth} className="w-full flex items-center justify-between gap-3 text-right group">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-orange-600 text-white flex items-center justify-center shrink-0 shadow-md shadow-orange-500/20">
                  <UserIcon className="w-4.5 h-4.5" />
                </div>
                <div className="min-w-0">
                  <div className="text-sm sm:text-base font-extrabold text-slate-900 leading-tight">أول مرة معانا؟ ابدأ من هنا 👋</div>
                  <div className="text-[11px] sm:text-xs font-semibold text-slate-600 mt-0.5 truncate">سجّل حسابك أو ادخل لحسابك في ثواني وابدأ طلبك</div>
                </div>
              </div>
              <div className="flex items-center gap-1.5 bg-orange-600 group-hover:bg-orange-500 text-white text-xs sm:text-sm font-extrabold px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl shadow-md shrink-0 transition-all group-active:scale-95">
                <span>ابدأ الآن</span><ArrowLeft className="w-4 h-4" />
              </div>
            </button>
          </div>
        )}
