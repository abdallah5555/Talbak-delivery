...
export const AdminUsersTab: React.FC<Props> = ({
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const trimmedPhone = addPhone.trim().replace(/\D/g, '');
    const trimmedPass = addPassword.trim();

    if (!trimmedName || !trimmedPhone) {
      setAddModalError('يرجى إدخال الاسم ورقم الهاتف.');
      return;
    }

    if (trimmedPhone.length < 10) {
      setAddModalError('يرجى إدخال رقم هاتف صحيح (11 رقم).');
      return;
    }

    if (!trimmedPass || trimmedPass.length < 6) {
      setAddModalError('كلمة المرور يجب أن تكون 6 أحرف أو أرقام على الأقل.');
      return;
    }
    ...
    try {
      ...
      const result = await onCreateUser(createdUser, trimmedPass);
      // Immediately clear sensitive password input from component state
      setAddPassword('');

      if (result && typeof result === 'object' && result.success === false) {
        setAddModalError(result.error || 'تعذر إنشاء الحساب.');
        return;
      }
      ...
      setAddModalError(err?.message || 'حدث خطأ أثناء إنشاء الحساب.');
    } finally {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await onUpdateUser(updated);
      if (res && typeof res === 'object' && res.success === false) {
        setEditModalError(res.error || 'تعذر حفظ التعديلات.');
        return;
      }

      setEditingUser(null);
    } catch (err: any) {
      setEditModalError(err?.message || 'حدث خطأ أثناء تعديل الحساب.');
    } finally {
          ...
          <tbody className="divide-y divide-slate-700/60">
            {filteredUsers.map((user) => (
              <tr key={user.id} className="hover:bg-slate-700/40 transition-colors">
                ...
                <td className="p-3 font-mono text-[11px] text-slate-400">••••••••</td>
                <td className="p-3">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 w-full max-w-lg text-white space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-1">رقم الموبايل (11 رقم)</label>
                <input
                  type="tel"
                  required
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-1">كلمة المرور الآمنة (6 خانات على الأقل)</label>
                <input
                  type="password"
                  required
                  placeholder="••••••••"
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 bg-orange-600 hover:bg-orange-500 font-bold py-2.5 rounded-xl text-xs disabled:opacity-50"
                >
                  {isSubmittingAdd ? 'جاري إنشاء الحساب...' : 'حفظ وتأكيد إضافة الحساب'}
                </button>
                <button
                  type="button"
                  disabled={isSubmittingAdd}
            ...
            <form onSubmit={handleSaveEdit} className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 bg-orange-600 hover:bg-orange-500 font-bold py-2.5 rounded-xl text-xs disabled:opacity-50"
                >
                  {isSubmittingEdit ? 'جاري الحفظ...' : 'تحديث وحفظ الحساب'}
                </button>
                <button
                  type="button"
                  disabled={isSubmittingEdit}
