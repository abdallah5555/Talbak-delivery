...
export async function sendTestPushNotification(user: User): Promise<{ success: boolean; message: string }> {
  const res = await sendPushNotification({ userId: user.id, role: user.role, title: 'طلبك دليفري 🛵 - إشعار تجريبي', body: `أهلاً بك يا ${user.name}! تم تفعيل إشعارات الهاتف بنجاح 🔔 وسيعمل التنبيه حتى لو كان التطبيق مغلقاً.`, type: 'system', url: '/' });
