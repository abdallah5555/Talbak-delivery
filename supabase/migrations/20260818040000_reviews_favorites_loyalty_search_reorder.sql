...
CREATE POLICY "store_reviews_customer_select" ON public.store_reviews FOR SELECT TO authenticated USING (customer_id = (SELECT auth.uid()) OR EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid()) AND ur.role = 'admin'));
CREATE POLICY "store_reviews_customer_insert" ON public.store_reviews FOR INSERT TO authenticated WITH CHECK (customer_id = (SELECT auth.uid()) AND EXISTS (SELECT 1 FROM public.orders o WHERE o.id = order_id AND o.customer_id = (SELECT auth.uid()) AND o.store_id = store_id AND o.status = 'delivered'));
CREATE POLICY "store_reviews_admin_all" ON public.store_reviews FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid()) AND ur.role = 'admin')) WITH CHECK (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid()) AND ur.role = 'admin'));
...
CREATE POLICY "favorites_owner_all" ON public.favorites FOR ALL TO authenticated USING (customer_id = (SELECT auth.uid())) WITH CHECK (customer_id = (SELECT auth.uid()));
...
BEGIN
  IF NEW.status = 'delivered' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF v_points > 0 THEN UPDATE public.users SET loyalty_points = loyalty_points + v_points, updated_at = now() WHERE id = NEW.customer_id; END IF;
  END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_award_loyalty_points ON public.orders;
CREATE TRIGGER trg_award_loyalty_points AFTER UPDATE OF status ON public.orders FOR EACH ROW EXECUTE FUNCTION public.award_loyalty_points_on_delivery();
...
RETURNS SETOF public.stores LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public, pg_temp AS $$
  SELECT s.* FROM public.stores s
  WHERE COALESCE(s.is_open, true)
    AND (nullif(trim(p_query), '') IS NULL OR s.search_vector @@ websearch_to_tsquery('simple', trim(p_query)) OR s.name ILIKE '%' || trim(p_query) || '%')
