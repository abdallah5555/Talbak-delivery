-- Phase 1: make user_roles authoritative for authorization while retaining users.role for compatibility.
CREATE OR REPLACE FUNCTION public.is_admin(user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles ur JOIN public.users u ON u.id = ur.user_id
    WHERE ur.user_id = $1 AND ur.role = 'admin' AND u.status = 'active'
  );
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles ur JOIN public.users u ON u.id = ur.user_id
    WHERE ur.user_id = $1 AND ur.role = 'driver' AND u.status = 'active'
  );
$$;
...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'Only active customers can create orders'; END IF;
