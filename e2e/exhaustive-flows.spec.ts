...
async function openTools(page: Page, label: RegExp) { await page.locator('.workspace-tools-trigger').filter({ hasText: label }).click(); }
 ...
 test.describe('customer exhaustive journeys', () => {
  ...
  test('checkout validation, address creation and payment guard work', async ({ page }) => {
    ...
    await page.locator('.checkout .x').click(); await page.locator('.drawer .x').click(); await page.getByRole('button', { name: /حسابي/ }).first().click(); await page.getByPlaceholder('العنوان بالتفصيل').fill(''); await page.getByRole('button', { name: /إضافة عنوان/ }).click(); await toastContains(page, /اكتب العنوان/); await page.getByPlaceholder('العنوان بالتفصيل').fill(`E2E New Address ${Date.now()}`); await page.getByRole('button', { name: /إضافة عنوان/ }).click(); await expect(page.locator('.address-list')).toContainText('E2E New Address');
  });
...
test.describe('merchant exhaustive operations', () => {
  test('store, menu and inventory operations execute and invalid inventory is rejected', async ({ page }, testInfo) => {
    const data = await runtime(); await login(page, 'merchant'); await page.getByRole('button', { name: 'المتجر', exact: true }).click(); const storeRow = page.locator('.pstore').filter({ hasText: `E2E Test Store ${data.runId}` }); await expect(storeRow).toHaveCount(1); const storeToggle = storeRow.getByRole('button'); await expect(storeRow.locator('.store-actions > span')).toHaveText('مفتوح'); await storeToggle.click(); await expect(storeRow.locator('.store-actions > span')).toHaveText('مغلق'); await storeToggle.click(); await expect(storeRow.locator('.store-actions > span')).toHaveText('مفتوح');
    await page.getByRole('button', { name: 'المنيو', exact: true }).click(); const nameInput = page.locator(`input[value="E2E Test Item ${data.runId}"]`).first(); await expect(nameInput).toBeVisible(); const menuRow = page.locator('.menu-table > div').filter({ has: nameInput }); await expect(menuRow).toHaveCount(1); await menuRow.getByRole('button', { name: 'حفظ' }).click(); await expect(page.locator('.toast')).toContainText('تم حفظ الصنف');
    const inventoryName = `E2E Inventory ${testInfo.project.name} ${Date.now()}`; const inv = page.locator('.merchant-inventory'); await expect(inv).toBeVisible(); await inv.getByPlaceholder('اسم الصنف / الخامة', { exact: true }).fill(inventoryName); await inv.getByPlaceholder('الوحدة', { exact: true }).fill('قطعة'); await inv.getByPlaceholder('حد التنبيه', { exact: true }).fill('5'); await inv.getByPlaceholder('تكلفة الوحدة', { exact: true }).fill('20'); await inv.getByRole('button', { name: /إضافة/ }).click(); await expect(inv).toContainText(inventoryName); const row = inv.locator('.inventory-card').filter({ hasText: inventoryName }); await expect(row).toHaveCount(1); await row.getByPlaceholder('+ / −', { exact: true }).fill('10'); await row.getByRole('button', { name: 'تطبيق' }).click(); await expect(row.locator(':scope > strong')).toContainText(/١٠|10/); await row.getByPlaceholder('+ / −', { exact: true }).fill('-3'); await row.getByRole('button', { name: 'تطبيق' }).click(); await expect(row.locator(':scope > strong')).toContainText(/٧|7/); await row.getByPlaceholder('+ / −', { exact: true }).fill('0'); await row.getByRole('button', { name: 'تطبيق' }).click(); await expect(inv.locator('.toast')).toContainText(/كمية زيادة أو نقص/);
  });
...
test.describe('driver exhaustive operations', () => {
  test('location, online/offline and safe controls', async ({ page }) => {
    await page.context().grantPermissions(['geolocation']); await page.context().setGeolocation({ latitude: 30.0444, longitude: 31.2357 }); await login(page, 'driver'); await expect(page.locator('.driver-ops')).toBeVisible(); await page.getByRole('button', { name: /تحديد موقعي/ }).click(); await expect(page.locator('.driver-location')).toContainText(/30\.0444/); const online = page.locator('.phead button.online'); await expect(online).toBeVisible(); if ((await online.innerText()).includes('أونلاين')) { await online.click(); await expect(online).toHaveText('أوفلاين'); } await online.click(); await expect(online).toHaveText('أونلاين'); await page.reload(); await expect(online).toHaveText('أونلاين'); await online.click(); await expect(online).toHaveText('أوفلاين'); await page.reload(); await expect(online).toHaveText('أوفلاين');
  });
